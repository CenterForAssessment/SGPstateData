...
function() {
		...
		for (vp in seq_along(content_areas)) {
			...
			studentGrowthPlot(
				...
				Report_Parameters=list(Current_Year=last.year, Content_Area=content_areas[vp], State="WIDA_GA", SGP_Targets=sgPlot.sgp.targets,
...
#######################
		...
		for (vp in seq_along(content_areas)) {
			...
			studentGrowthPlot(
				...
				Cutscores=sgPlot.cutscores[[content_areas[vp]]],
				Years=rev(sgPlot.years),
				Report_Parameters=list(Current_Year=last.year, Content_Area=content_areas[vp], State="WIDA_GA_SPANISH", SGP_Targets=sgPlot.sgp.targets,
		...
		# grid.text(x=0.02, y=0.65, "Para obtener m\u{E1}s informaci\u{F3}n visite www.cde.state.co.us/accountability/englishlanguageproficiencygrowth o llamar El Departamento de",
			...
			copyright.text <- "Producido por El Departamento de Educaci\u{F3}n de Georgia (GaDOE) y El Centro de Evaluaci\u{F3}n, Inc. Distribuido por GaDOE."
